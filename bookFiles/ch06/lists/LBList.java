...
public class LBList<T> implements ListInterface<T> {
  ...
  public boolean remove(T target)
    ...
    find(target);
    if (found) {
      if (front == location) front = front.getLink(); // remove first node
      else previous.setLink(location.getLink()); // remove node at location
  ...
  public void add(int index, T element)
    ...
    if ((index < 0) || (index > size()))
      throw new IndexOutOfBoundsException(
          "Illegal index of " + index + " passed to LBList add method.\n");
    ...
    {
      LLNode<T> node = front;
      for (int i = 0; i < (index - 1); i++) node = node.getLink();
  ...
  public T set(int index, T newElement)
    ...
    if ((index < 0) || (index >= size()))
      throw new IndexOutOfBoundsException(
          "Illegal index of " + index + " passed to LBList set method.\n");

    LLNode<T> node = front;
    for (int i = 0; i < index; i++) node = node.getLink();
    T hold = node.getInfo();
  ...
  public T remove(int index)
    ...
    if ((index < 0) || (index >= size()))
      throw new IndexOutOfBoundsException(
          "Illegal index of " + index + " passed to LBList remove method.\n");
