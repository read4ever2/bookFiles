...
public class SortedABList<T> implements ListInterface<T> {
  ...
  protected void find(T target)
    ...
    location = 0;
    found = false;
    if (!isEmpty()) recFind(target, 0, numElements - 1);
  }
  ...
  protected void recFind(T target, int first, int last)
    ...
    if (first > last) {
      ...
      if (result > 0) location++; // adjust location to indicate insert index
    } else {
  ...
  public boolean add(T element)
    ...
    for (int index = numElements; index > location; index--) list[index] = list[index - 1];
  ...
  public boolean remove(T target)
    ...
    find(target);
    if (found) {
      for (int i = location; i <= numElements - 2; i++) list[i] = list[i + 1];
  ...
  public void add(int index, T element)
    ...
    throw new UnsupportedOperationException(
        "Unsupported index-based add attempted on sorted list.");
  }
  ...
  public T set(int index, T newElement)
      ...
      {
    throw new UnsupportedOperationException(
        "Unsupported index-based set attempted on sorted list.");
  }
  ...
  public T get(int index)
    ...
    if ((index < 0) || (index >= size()))
      throw new IndexOutOfBoundsException(
          "illegal index of " + index + " passed to ABList get method.\n");
  ...
  public int indexOf(T target)
    ...
    if (found) {
      ...
      while ((location != 0) && (comp.compare(list[location - 1], target) == 0)) location--;
      return location;
    } else return -1;
  }
  ...
  public T remove(int index)
    ...
    if ((index < 0) || (index >= size()))
      throw new IndexOutOfBoundsException(
          "illegal index of " + index + " passed to ABList remove method.\n");

    T hold = list[index];
    for (int i = index; i < numElements - 1; i++) list[i] = list[i + 1];
    list[numElements - 1] = null;
    numElements--;
    return hold;
  }
  ...
  public Iterator<T> iterator()
    ...
    return new Iterator<T>() {
      ...
      public T next()
        ...
        if (!hasNext())
          throw new IndexOutOfBoundsException(
              "illegal invocation of next " + " in LBList iterator.\n");
        previousPos++;
      ...
      public void remove()
          ...
          {
        for (int i = previousPos; i <= numElements - 2; i++) list[i] = list[i + 1];
        list[numElements - 1] = null;
        numElements--;
        previousPos--;
      }
