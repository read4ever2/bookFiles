  ...
  public static int evaluate(String expression) {
    ...
    while (tokenizer.hasNext()) {
      if (tokenizer.hasNextInt()) {
        ...
        if (stack.isFull()) throw new PostFixException("Too many operands-stack overflow");
        stack.push(value);
      } else {
        // Process operator.
        ...
        if (!(operator.equals("/")
            || operator.equals("*")
            || operator.equals("+")
            || operator.equals("-"))) throw new PostFixException("Illegal symbol: " + operator);
    ...
    if (!stack.isEmpty()) throw new PostFixException("Too many operands-operands left over");
