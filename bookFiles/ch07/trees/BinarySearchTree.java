...
public class BinarySearchTree<T> implements BSTInterface<T> {
  ...
  public int size2()
    ...
    if (root != null) {
      ...
      while (!nodeStack.isEmpty()) {
        ...
        nodeStack.pop();
        count++;
        if (currNode.getLeft() != null) nodeStack.push(currNode.getLeft());
        if (currNode.getRight() != null) nodeStack.push(currNode.getRight());
      }
    }
    return count;
  }
  ...
  private boolean recContains(T target, BSTNode<T> node)
      ...
      {
    if (node == null) return false; // target is not found
  ...
  private BSTNode<T> recAdd(T element, BSTNode<T> node)
    ...
    else node.setRight(recAdd(element, node.getRight())); // Add in right subtree
    return node;
  }
  ...
  */
  ...
  public Iterator<T> getIterator(BSTInterface.Traversal orderType)
    ...
    if (orderType == BSTInterface.Traversal.Preorder) preOrder(root, infoQueue);
    else if (orderType == BSTInterface.Traversal.Inorder) inOrder(root, infoQueue);
    else if (orderType == BSTInterface.Traversal.Postorder) postOrder(root, infoQueue);
