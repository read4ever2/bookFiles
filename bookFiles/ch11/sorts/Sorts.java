...
public class Sorts {
  ...
  static void initValues()
    ...
    for (int index = 0; index < SIZE; index++) values[index] = Math.abs(rand.nextInt()) % 100;
  }
  ...
  static void selectionSort()
      ...
      {
    int endIndex = SIZE - 1;
    for (int current = 0; current < endIndex; current++) swap(current, minIndex(current, endIndex));
  }
  ...
  static void bubbleUp(int startIndex, int endIndex)
    ...
    for (int index = endIndex; index > startIndex; index--)
      if (values[index] < values[index - 1]) swap(index, index - 1);
  }
  ...
  static void insertionSort()
      ...
      {
    for (int count = 1; count < SIZE; count++) insertItem(0, count);
  }
  ...
  static void heapSort()
    ...
    for (index = SIZE / 2 - 1; index >= 0; index--) reheapDown(values[index], index, SIZE - 1);
